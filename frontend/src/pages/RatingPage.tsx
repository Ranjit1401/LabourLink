...
export default function RatingPage() {
  ...
  // FIXED: Added 'language' to the destructuring here
  ...
  const handleSubmit = async () => {
    if (rating === 0) {
      showToast(t(language, 'pleaseSelectStarRating'), 'error');
      return;
    }
    if (comment.length < 20) {
      showToast(t(language, 'pleaseWriteReview'), 'error');
      return;
    }
    ...
    try {
      ...
      setSubmitted(true);
      showToast(t(language, 'ratingSuccessMessage'), 'success');
          ...
          <div className="md:col-span-4">
            <div className="bg-surface-container-lowest rounded-xl p-6 shadow-sm border border-outline-variant/10 sticky top-28">
              ...
              <div className="text-center">
                ...
                <p className="text-on-surface-variant font-medium text-sm mb-4">{t(language, 'masterElectrician')}</p>
                <div className="bg-surface-container rounded-lg p-3 text-left">
                  <div className="flex justify-between text-xs font-semibold uppercase tracking-widest text-on-surface-variant mb-1">
                    <span>{t(language, 'projectLabel')}</span>
          ...
          <div className="md:col-span-8">
            {submitted ? (
              <div className="bg-surface-container rounded-xl p-12 text-center">
                ...
                <h2 className="font-headline text-3xl font-bold text-on-surface mb-4">{t(language, 'ratingThankYou')}</h2>
                <p className="text-on-surface-variant text-lg">{t(language, 'ratingSuccessMessage')}</p>
              </div>
            ) : (
              <div className="bg-surface-container rounded-xl p-8 space-y-8">
                ...
                <div className="space-y-3">
                  <label className="font-headline text-sm font-bold text-on-surface block uppercase tracking-wider">{t(language, 'highlightStrengths')}</label>
                ...
                {/* Review Text */}
                <div className="space-y-3">
                  <label className="font-headline text-lg font-bold text-on-surface block" htmlFor="review-text">{t(language, 'reviewComments')}</label>
                  <div className="relative">
                    <textarea
                      ...
                      id="review-text"
                      placeholder={t(language, 'reviewPlaceholder', { firstName: workerFirstName })}
                      rows={5}
                      value={comment}
